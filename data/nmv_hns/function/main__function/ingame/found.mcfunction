...
execute as @a[scores={turnToSeeker=1},team=hider,tag=!antiDeductionRankScore] run tellraw @s ["§c-",{"score": {"name": "found","objective": "rankScoreGet"},"color": "red"}," §fRank Score Points (found)"]
execute as @a[scores={turnToSeeker=1},team=hider,tag=antiDeductionRankScore] run tellraw @s ["§c-0"," §fRank Score Points (found, use ANTI-DEDUCTION RANK SCORE)"]
